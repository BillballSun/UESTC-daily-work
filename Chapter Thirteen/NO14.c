...
bool Are_anagrams(const char *word1, const word2)
{
	int letter1[Letters_amount]={0},letter2[Letters_amount]={0};
	while(*word1)
		letter1[toupper(*word1++)-'A']++;
	while(*word2)
		letter1[toupper(*word2++)-'A']++;
	bool result=true;
	for(int i=0;result&&i<=Letters_amount-1;i++)
		if(letter1[i]!=letter2[i]) result=false;
	return result;
}
