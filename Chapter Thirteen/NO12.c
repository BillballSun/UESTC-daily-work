...
bool Is_try_again(void);
void Main_program(void);
int Capture_word(char (*)[Max_word_length+1],char *);
bool Is_last_sysmbol(char);
...
void Main_program(void)
{
	char storage[Max_word_amount][Max_word_length+1];
	char last_symbol='\0';
	int word_amount=Capture_word(storage,&last_symbol);
	printf("In reversed sentense, it is: ");
	for(int i=word_amount-1;i>=1;i--)
		printf("%s ",storage[i]);
	printf("%s",*(storage+0));
	if(last_symbol) putchar(last_symbol);
	putchar('\n');
}

bool Is_last_sysmbol(char ch)
{
	return ch=='.'||ch=='/?'||ch=='!';
}

int Capture_word(char (*storage)[Max_word_length+1],char *last_symbol_point)
//return how many word captured
{
	char ch;
	int word_amount=0;
	bool checkpoint=false;
	printf("Please enter a sentense:");
	while(!checkpoint&&(ch=getchar())!='\n'&&word_amount<Max_word_amount)
	{
		if(Is_last_sysmbol(ch))
		{
			*last_symbol_point=ch;
			checkpoint=true;
		}
		if(!isspace(ch))
		{
			char *temp=*(storage+word_amount);
			int temp_counter=Max_word_length;
			do
			{
				*temp++=ch;
				temp_counter--;
				ch=getchar();
			}while(temp_counter>0&&!isspace(ch)&&!Is_last_sysmbol(ch));
			*temp='\0';
			if(Is_last_sysmbol(ch))
			{
				*last_symbol_point=ch;
				...
				while(!isspace(ch=getchar())&&!Is_last_sysmbol(ch)) continue;
			if(Is_last_sysmbol(ch))
			{
				*last_symbol_point=ch;
				checkpoint=true;
...
void Show_start_info(void)
{
	...
	printf("Copyright (c) 2016, 1998  CaptainAllred Incorperated.\n\n");
	printf("Ways To Use:\n");
	printf("Program will reverse a sentense automatically.\n");
	Print_divide_line();
}
