...
void Show_result(void)
{
	bool straight,flush,four,three_color_same,full_house,a_straight,big_same_straight;
	short pairs;
	big_same_straight=Is_big();
	 ...
	 */
    printf("\n");
	if (big_same_straight) printf("Unbelievable, you got big same straight.");
	else if (straight&&flush) printf("You get Straight Flush, what some nice cards.");
	...
	else if (pairs==1) printf("Only one pair.");
	else if (a_straight) printf("Hei, you got small a straight.");
...
bool Is_big(void)
{
	unsigned short order_amount[5];
	bool result;
	Get_amount_order(order_amount);
	Sort_order(order_amount);
	result=false;
	if (Is_flush())
		if(Is_straight())
			if(order_amount[0]==8u)
				result=true;
	return result;
}
